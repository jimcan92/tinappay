<script lang="ts">
	...
	let editingItem = $derived(
		inventoryState.products.items.find((p) => p.id === editingId) as ProductsResponse<any> | undefined
	);

	let categoryOptions = $derived([
		{ value: '', label: 'All Categories' },
		...categoriesState.productCategories.map((c) => ({ value: c.id, label: c.name })),
	...
	let filteredProducts = $derived(
		(inventoryState.products.items as ProductsResponse<any>[]).filter((p) => {
	...
	onMount(async () => {
		...
		if (editId) {
			const target = inventoryState.products.items.find((p) => p.id === editId) as ProductsResponse<any> | undefined;
	...
	function openAdd() {
		...
		form = { 
			name: '', 
			category: categoriesState.productCategories[0]?.id || '', 
			price: 0, 
			description: '', 
			unit: 'pcs',
			active: true,
	...
	function openEdit(item: ProductsResponse<any>) {
		editingId = item.id;
		form = { 
			name: item.name || '', 
	...
	async function handleSave() {
		...
		try {
			...
			if (dialogMode === 'add') {
				await inventoryState.createProduct(fd);
				toastState.success('Product created.');
			} else if (editingId) {
				await inventoryState.updateProduct(editingId, fd);
				toastState.success('Product updated.');
			}
			closeDialog();
		} catch (e: any) {
			const msg = e?.data ? Object.values(e.data).map((f: any) => f?.message).filter(Boolean).join(', ') : '';
			toastState.error(msg || 'Failed to save product.');
		} finally {
			saving = false;
		}
	}
	...
	async function handleDelete() {
		...
		saving = true;
		try {
			await inventoryState.deleteProduct(editingId);
			toastState.success('Product deleted.');
			closeDialog();
		} catch {
			toastState.error('Failed to delete product.');
		} finally {
			saving = false;
			confirmDelete = false;
		}
...
<svelte:head><title>Products | Management</title></svelte:head>
...
<div class="no-scrollbar h-full overflow-y-auto px-6 py-8 md:px-10">
	...
	<div class="mb-4 flex flex-col gap-3 md:flex-row md:items-center">
		<div class="relative flex-1">
			...
			<input bind:value={searchQuery} type="text" placeholder="Search products..." class="h-12 w-full rounded-full border-none bg-surface-container-low pl-12 pr-4 text-sm font-medium shadow-inner outline-none focus:ring-2 focus:ring-primary/20" />
		</div>
		<div class="flex w-full items-center gap-3 md:w-auto">
			<Select
				value={selectedCategoryId}
				onchange={(v) => { if (v === '__new__') goto('/management/products/categories'); else selectedCategoryId = v; }}
			...
			<SignatureButton onclick={openAdd} size="md" class="flex-1 md:flex-none">
				...
				<span class="truncate">New Product</span>
			</SignatureButton>
		</div>
	</div>
					...
					>
						<td class="px-6 py-4">
							<div class="flex items-center gap-3">
								<InventoryImage record={product} size="md" />
						...
						<td class="px-4 py-4">
							<code class="text-[10px] font-bold text-on-surface-variant">{product.barcode || '—'}</code>
						</td>
						<td class="px-4 py-4 text-right font-serif font-black text-on-surface">
							₱{product.price?.toFixed(2) ?? '0.00'}
						</td>
						<td class="px-4 py-4 max-w-[200px]">
							<p class="truncate text-sm text-on-surface-variant/60">{product.description || '—'}</p>
						</td>
					</tr>
				{/snippet}
			</PaginatedTable>
...
<Dialog
	...
	title={dialogMode === 'add' ? 'New Product' : 'Edit Product'}
	onClose={closeDialog}
	size="md"
>
	<div class="space-y-6">
		...
		<div class="grid grid-cols-[auto_1fr] gap-6">
				...
				<div class="h-[140px] w-[140px] overflow-hidden rounded-3xl border-2 border-outline-variant/20 bg-surface-container-low shadow-inner transition-all group-hover:border-primary/30">
					...
					{:else}
						<div class="flex h-full w-full items-center justify-center bg-primary/5">
							<span class="material-symbols-outlined text-5xl text-primary/20">bakery_dining</span>
						</div>
					{/if}
				</div>
			...
			<div class="flex flex-col justify-center gap-5">
				<div class="space-y-1.5">
					<label for="pd-name" class="px-1 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Product Name</label>
					<input id="pd-name" bind:value={form.name} type="text" placeholder="e.g. Sourdough Loaf" class="h-12 w-full rounded-2xl border-none bg-surface-container-low px-5 text-sm font-bold shadow-inner outline-none focus:ring-2 focus:ring-primary/20" />
				</div>
				
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-1.5">
						<p class="px-1 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Status</p>
						<label class="flex h-11 w-full cursor-pointer items-center gap-3 rounded-xl bg-surface-container-low px-4 shadow-inner transition-colors hover:bg-surface-container">
							<div class="relative inline-flex items-center">
								<input type="checkbox" bind:checked={form.active} class="peer sr-only" />
								<div class="h-5 w-9 rounded-full bg-on-surface/10 transition-colors peer-checked:bg-primary"></div>
								<div class="absolute left-1 top-1 h-3 w-3 rounded-full bg-white transition-transform peer-checked:translate-x-4"></div>
							</div>
							<span class="text-[10px] font-black uppercase tracking-widest {form.active ? 'text-primary' : 'text-on-surface-variant/40'}">
		...
		<div class="grid grid-cols-2 gap-4">
			<div class="space-y-1.5">
				...
				<Select
					...
					onchange={(v) => { if (v === '__new__') goto('/management/products/categories'); }}
					placeholder="Select category"
				/>
			</div>
			<div class="space-y-1.5">
				<label for="pd-price" class="px-1 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Price (₱)</label>
				<input id="pd-price" bind:value={form.price} type="number" step="0.01" min="0" class="h-11 w-full rounded-xl border-none bg-surface-container-low px-4 text-sm font-bold shadow-inner outline-none focus:ring-2 focus:ring-primary/20" />
			</div>
		</div>

		<div class="space-y-1.5">
			<label for="pd-barcode" class="px-1 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Barcode / SKU</label>
			<div class="relative">
				<input id="pd-barcode" bind:value={form.barcode} type="text" placeholder="Scan or enter code" class="h-12 w-full rounded-2xl border-none bg-surface-container-low pl-5 pr-12 text-sm font-bold shadow-inner outline-none focus:ring-2 focus:ring-primary/20" />
				{#if scannerSupported}
					<button 
						onclick={openScanner}
		...
		<div class="space-y-1.5">
			<label for="pd-desc" class="px-1 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Description</label>
			<textarea id="pd-desc" bind:value={form.description} placeholder="Product notes..." rows="2" class="w-full resize-none rounded-2xl border-none bg-surface-container-low px-5 py-4 text-sm font-medium shadow-inner outline-none focus:ring-2 focus:ring-primary/20"></textarea>
		</div>
	</div>
