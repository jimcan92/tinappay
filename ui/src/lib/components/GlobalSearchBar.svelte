<script lang="ts">
	...
	let filteredHistory = $derived(
		hasQuery ? history.filter((h) => h.toLowerCase().includes(query.trim().toLowerCase())) : history
	);
	...
	async function fetchResults(q: string) {
		try {
			...
			const [products, supplies, categories, suppliers, users] = await Promise.allSettled([
				pb
					.collection('products')
					.getList(1, 5, { filter: nameFilter, sort: 'name', requestKey: `gs-products-${q}` }),
				pb
					.collection('supplies')
					.getList(1, 5, { filter: nameFilter, sort: 'name', requestKey: `gs-supplies-${q}` }),
				pb
					.collection('categories')
					.getList(1, 5, { filter: nameFilter, sort: 'name', requestKey: `gs-categories-${q}` }),
				pb
					.collection('suppliers')
					.getList(1, 5, {
						filter: `name ~ "${q}" || contact_person ~ "${q}"`,
						sort: 'name',
						requestKey: `gs-suppliers-${q}`
					}),
				pb
					.collection('users')
					.getList(1, 5, {
			...
			addSection('categories', 'Categories', 'category', categories, (r) => ({
				...
				icon: 'category',
				href: r.type === 'supply'
					? '/management/supplies/categories'
...
<div class="search-bar-root relative {className}">
	...
	<div
		class="flex items-center gap-3 rounded-full px-4 py-2 transition-all duration-200
			{focused ? 'bg-surface-container-low ring-1 ring-primary/30' : 'bg-surface-container-low hover:bg-surface-container-low'}"
	>
		{#if loading}
			<span
		...
		<input
			...
			onblur={onBlur}
			onkeydown={onKeydown}
			class="font-body placeholder:text-on-surface-variant/50 w-full border-none bg-transparent text-sm focus:ring-0 focus:outline-none"
		...
		{#if query}
			<button
				onclick={clearQuery}
				class="text-on-surface-variant/50 shrink-0 transition-colors hover:text-on-surface"
	...
	{#if showDropdown}
		<div
			...
			class="search-dropdown absolute top-full right-0 left-0 z-50 mt-2 overflow-hidden rounded-2xl border border-outline-variant/10 bg-surface-container-lowest/97 shadow-2xl backdrop-blur-xl"
			role="listbox"
		>
			<!-- Live results -->
			{#if hasQuery}
					...
					<div class="space-y-3 px-4 pt-4 pb-2">
						{#each [1, 2, 3] as _}
							<div class="flex items-center gap-3">
								<div class="bg-on-surface/6 h-7 w-7 animate-pulse rounded-full"></div>
								<div class="flex-1 space-y-1.5">
									<div class="bg-on-surface/6 h-3 w-2/3 animate-pulse rounded-full"></div>
									<div class="bg-on-surface/5 h-2.5 w-1/3 animate-pulse rounded-full"></div>
								</div>
							</div>
						{/each}
					</div>
				{:else if !loading && !hasResults}
					...
					<div class="flex flex-col items-center gap-2 px-4 py-10 text-center">
						<span class="material-symbols-outlined text-on-surface-variant/30 text-4xl">
							search_off
						</span>
						<p class="text-on-surface-variant/60 text-sm font-semibold">No results found</p>
						<p class="text-on-surface-variant/40 text-xs">
					...
					{#each sections as section, si}
						...
						<div class="border-b border-outline-variant/10 last:border-0">
							...
							<div class="flex items-center gap-2 px-4 pt-3 pb-1">
								<span class="material-symbols-outlined text-on-surface-variant/50 text-sm">
									{section.icon}
								</span>
								<span
									class="text-on-surface-variant/50 text-[10px] font-black tracking-widest uppercase"
								>
									{section.label}
								</span>
								<span class="text-on-surface-variant/40 ml-auto text-[10px] font-bold">
							...
							{#each section.items as item, ii}
								...
								<div
									...
									class="group flex cursor-pointer items-center gap-3 px-4 py-2.5 transition-all
										{activeIndex === flatIdx ? 'bg-primary/8 text-primary' : 'text-on-surface hover:bg-surface-container'}"
								>
									<div
									...
									<div class="min-w-0 flex-1">
										...
										{#if item.sublabel}
											<p class="text-on-surface-variant/60 mt-0.5 truncate text-[11px]">
												{item.sublabel}
											</p>
										{/if}
									</div>
									...
									<span
										class="material-symbols-outlined text-on-surface-variant/30 shrink-0 text-sm opacity-0 transition-all group-hover:opacity-100 {activeIndex ===
										flatIdx
			...
			{#if filteredHistory.length > 0}
				<div class="border-t border-outline-variant/10 {hasQuery && hasResults ? '' : 'border-t-0'}">
					<div class="flex items-center justify-between px-4 pt-3 pb-1">
						<span class="text-on-surface-variant/50 text-[10px] font-black tracking-widest uppercase">
							{hasQuery ? 'Matching History' : 'Recent Searches'}
						</span>
						{#if !hasQuery}
							<button
								onclick={clearHistory}
								class="text-[10px] font-bold tracking-wider text-primary/60 uppercase transition-colors hover:text-primary"
							>
								Clear all
							</button>
						{/if}
					...
					{#each filteredHistory as entry, hi}
						...
						>
							...
							>
								<span class="material-symbols-outlined text-on-surface-variant/50 shrink-0 text-sm">
									history
								</span>
							...
							<span
								...
								class="material-symbols-outlined text-on-surface-variant/30 shrink-0 cursor-pointer text-sm opacity-0 transition-all group-hover:opacity-100 hover:!text-error"
			...
			{#if showDropdown && (hasResults || filteredHistory.length > 0)}
				<div class="flex items-center gap-3 border-t border-outline-variant/10 px-4 py-2">
					<span class="text-on-surface-variant/40 text-[10px]">
						<kbd class="rounded bg-surface-container-low px-1 py-0.5 font-mono text-[9px]">↑↓</kbd> navigate
					</span>
					<span class="text-on-surface-variant/40 text-[10px]">
						<kbd class="rounded bg-surface-container-low px-1 py-0.5 font-mono text-[9px]">Enter</kbd> select
					</span>
					<span class="text-on-surface-variant/40 text-[10px]">
						<kbd class="rounded bg-surface-container-low px-1 py-0.5 font-mono text-[9px]">Esc</kbd> close
					</span>
				</div>
			{/if}
		</div>
